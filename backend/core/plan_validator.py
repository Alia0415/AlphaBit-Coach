...
def _validate_step_contract(
    ...
    elif step.agent == AgentId.QUANT:
        _validate_quant_inputs(step)
...
def _validate_quant_inputs(step: PlanStep) -> None:
    mode = step.inputs.get("analysis_mode")
    if mode is not None and mode not in _QUANT_ANALYSIS_MODES:
        raise PlanValidationError(
